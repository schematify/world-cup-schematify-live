...
Simulate random live scores on 3 matches by default. The matches are selected once at startup and the same score nodes are updated every tick. Changed scores use `base/alert` for 15 seconds, then decay to `base/info` while the match remains running:
